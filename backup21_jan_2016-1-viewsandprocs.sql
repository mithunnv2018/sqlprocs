...
/* Function  structure for function  `fun_decrypt` */

/*!50003 DROP FUNCTION IF EXISTS `fun_decrypt` */;
DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `fun_decrypt`(inparam longblob) RETURNS varchar(500) CHARSET utf8
    DETERMINISTIC
BEGIN
	
	return AES_DECRYPT(inparam,'atulkulve');
    END */$$
DELIMITER ;

/* Function  structure for function  `fun_encrypt` */

/*!50003 DROP FUNCTION IF EXISTS `fun_encrypt` */;
DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `fun_encrypt`(inencrypt varchar(500)) RETURNS longblob
    DETERMINISTIC
BEGIN
	return AES_ENCRYPT(inencrypt,'atulkulve');
    END */$$
DELIMITER ;
...
CONCAT(IF ( DATA_TYPE = 'datetime',CONCAT('UNIX_TIMESTAMP(maina.',COLUMN_NAME,')*1000'),CONCAT('maina.',COLUMN_NAME,''))),'),\'",\''))
 ...
 WHILE @x1  < @countpk DO
	...
	SET @subpkname = JSON_UNQUOTE(JSON_EXTRACT(JSON_EXTRACT(@subtablesarr , '$[*].subpkname'),CONCAT('$[',@x1,']')));
	  ...
	  SET @a=(SELECT DISTINCT GROUP_CONCAT(CONCAT('\'', COLUMN_NAME,'\',','sub',@x1,'.',IF ( DATA_TYPE = 'datetime',CONCAT('UNIX_TIMESTAMP(',COLUMN_NAME,')*1000'),COLUMN_NAME)))
	  FROM INFORMATION_SCHEMA.COLUMNS
