...
BEGIN
 ...
 SET @primarypk='ofcprojectpost_id';
 SET @primarytable='tbl_ofcprojectpost_details';
SET @mainconcatstart = 'SELECT CONCAT(\'[\', (SELECT GROUP_CONCAT(CONCAT(\'{';
SET @maina=(SELECT DISTINCT GROUP_CONCAT(CONCAT('\'"', COLUMN_NAME,'":"\',IF(ISNULL(maina.',COLUMN_NAME,'),\'-\',',CONCAT(IF ( DATA_TYPE = 'datetime',CONCAT('UNIX_TIMESTAMP(maina.',COLUMN_NAME,')*1000'),CONCAT('maina.',COLUMN_NAME))),')'))
	  FROM INFORMATION_SCHEMA.COLUMNS
	  WHERE table_name = @primarytable AND table_schema = DATABASE());
 SET @countpk = JSON_LENGTH(JSON_EXTRACT(subtables , '$[0].subtables'));
 SET @x1=0;
 SET @subtablesarr = JSON_UNQUOTE(JSON_EXTRACT(subtables , '$[0].subtables'));
 -- {"subtblname":"","mainpkname":"","subkey":"","subpkname":""}
 WHILE @x1  < @countpk DO
	...
	SET @subpkname = JSON_UNQUOTE(JSON_EXTRACT(JSON_EXTRACT(@subtablesarr , '$[*].subpkname'),CONCAT('$[',@x1,']')));
	  ...
	  -- final
	  SET @subtablefinalquery = CONCAT('","',@subkey,'":\')',@subprefix,@a,@subend);
	...
	SET @mainsuba = CONCAT(@subtablefinalquery,'FROM ',@primarytable,' maina where maina.',@primarypk,'=maina.',@primarypk);
	...
	IF @truesession THEN
		-- SET istrue=TRUE;
		 ...
		 ELSE
			...
			SET outparam=(SELECT CONCAT('[', (SELECT GROUP_CONCAT(CONCAT('{"ofcprojectpost_id"',':"',IF(ISNULL(a.ofcprojectpost_id),'-',a.ofcprojectpost_id),'","','subcat_id','":"',IF(ISNULL(a.subcat_id),'-',a.subcat_id),'","','experience_id','":"',IF(ISNULL(a.experience_id),'-',a.experience_id)
,'ofcprojectpost_budget','":"',IF(ISNULL(a.ofcprojectpost_budget),'-',a.ofcprojectpost_budget)
,'ofcprojectpost_budget','":"',IF(ISNULL(a.ofcprojectpost_budget),'-',a.ofcprojectpost_budget)
,'ofcprojectpost_negotiable','":"',IF(ISNULL(a.ofcprojectpost_negotiable),'-',a.ofcprojectpost_negotiable)
,'ofcprojectpost_title','":"',IF(ISNULL(a.ofcprojectpost_title),'-',a.ofcprojectpost_title)
...
,'ofcprojectpost_invitefreelancers','":"',IF(ISNULL(a.ofcprojectpost_invitefreelancers),'-',a.ofcprojectpost_invitefreelancers)
,'ofcprojectpost_public','":"',IF(ISNULL(a.ofcprojectpost_public),'-',a.ofcprojectpost_public)
,'country_id','":"',IF(ISNULL(a.country_id),'-',a.country_id)
,'language_id','":"',IF(ISNULL(a.language_id),'-',a.language_id)
,'ofcprojectpost_projectamount','":"',IF(ISNULL(a.ofcprojectpost_projectamount),'-',a.ofcprojectpost_projectamount)
,'ofcprojectpost_advanceamount','":"',IF(ISNULL(a.ofcprojectpost_advanceamount),'-',a.ofcprojectpost_advanceamount)
,'ofcprojectpost_finalpaidamount','":"',IF(ISNULL(a.ofcprojectpost_finalpaidamount),'-',a.ofcprojectpost_finalpaidamount)
,'ofcprojectpost_ispaymenttermsagreed','":"',IF(ISNULL(a.ofcprojectpost_ispaymenttermsagreed),'-',a.ofcprojectpost_ispaymenttermsagreed)
...
,'","filearr":'
 ...
 ,(SELECT CONCAT('[',GROUP_CONCAT( json_object('skill_id',x2.skill_id,'skill_name',x2.skill_name)) ,']}')
FROM tbl_ofcprojectpost_skill x1, tbl_skill_master x2 WHERE x1.ofcprojectpost_id=a.ofcprojectpost_id AND x1.skill_id = x2.skill_id))
  FROM tbl_ofcprojectpost_details a, tbl_ofcmembers_master c,tbl_experience_master d WHERE a.ofcmembers_id=c.ofcmembers_id AND a.experience_id = d.experience_id and a.ofcprojectpost_id=projectpostid ORDER BY a.createdate DESC)
  ,']')
  );
        ...
        -- SET istrue=TRUE;
         ...
         ELSE
            CALL proc_runningid('PPOST',@p);
             ...
             (ofcprojectpost_id,ofcmembers_id,subcat_id,experience_id,ofcprojectpost_budget,ofcprojectpost_title,
             ofcprojectpost_description,ofcprojectpost_timeframe,ofcprojectpost_typepreference
             ,ofcprojectpost_kind,ofcprojectpost_budgetamount,ofcprojectpost_estimatestartdate,country_id,language_id,
             createdate,STATUS)
             VALUES(@p,
             @memid,
             JSON_UNQUOTE(JSON_EXTRACT(inparam , '$[0].subcat_id')),
             JSON_UNQUOTE(JSON_EXTRACT(inparam , '$[0].experience_id')),
             ...
             JSON_UNQUOTE(JSON_EXTRACT(inparam , '$[0].country_id')),
             JSON_UNQUOTE(JSON_EXTRACT(inparam , '$[0].language_id')),            
             NOW(),'active');
