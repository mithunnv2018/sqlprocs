...
BEGIN
	call proc_common_auto_drpjson('keysubcat_id','keysubcat_name','vw_subcat_keyword_details','subcat_id=\'',subcatid,'\'',outparam); -- SELECT * FROM vw_subcat_keyword_details
    END */$$
DELIMITER ;
...
BEGIN
	call proc_common_auto_drpjson('skill_id','skill_name','vw_skill_subcat_maincat_details','subcat_id=\'',subcatid,'\'',outparam); -- SELECT * FROM vw_skill_subcat_maincat_details
    END */$$
DELIMITER ;
		...
		-- SET istrue=TRUE;
		 ...
		 ELSE
			CALL proc_runningid('MEDU',@p);
		 	...
		 	(ofcmembereducation_id,ofcmembers_id,education_id,createdate,STATUS)
		 	VALUES(@p,
		 	@memid,
...
BEGIN
	call proc_common_selectjson('vw_membereducation_education','ofcmemsession_id=\'',ofcmembersessionid,'\'',outparam);
    END */$$
DELIMITER ;
...
BEGIN
	call proc_common_selectjson('vw_memberemployment_maincat','ofcmemsession_id=\'',ofcmembersessionid,'\'',outparam);
    END */$$
DELIMITER ;
...
BEGIN
	call proc_common_selectjson('vw_memberexperience_maincat','ofcmemsession_id=\'',ofcmembersessionid,'\'',outparam);
    END */$$
DELIMITER ;
...
BEGIN
	call proc_common_selectjson('vw_membermyproject_maincat','ofcmemsession_id=\'',ofcmembersessionid,'\'',outparam);
    END */$$
DELIMITER ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_vw_popuplarsearch_homepage_filesimages_selectjson`(OUT result json,OUT outparam json)
label1:
BEGIN
 DECLARE errcount INT;
DECLARE errno INT;
